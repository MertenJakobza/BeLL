...
def take_image():
    ...
    # img = cv2.imread("testProgramm2.png")
    # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    # img = cv2.imread("test.png")
    # img = cv2.resize(img, (mainui.Lcamera_preview.height(), mainui.Lcamera_preview.height()))
    # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
...
def classify():
    ...
    print(len(features), max(features))

    results = runner.classify(features)
    img2 = cropped.copy()
    classification_result = []
    bounding_boxes = results["result"]["bounding_boxes"]

    for bb in bounding_boxes:
        img2 = cv2.rectangle(img2, (bb['x'], bb['y']), (bb['x'] + bb['width'], bb['y'] + bb['height']), (255, 0, 0), 1)
        print(bb)
        classification_result.append(bb)

    if (runner):
        runner.stop()
    display_image(img2)
    # clasification_result = results
    if len(bounding_boxes) == 0:
        print("Keine Bounding-Boxen gefunden!")
        dialog = QMessageBox()
        dialog.setIcon(QMessageBox.Critical)
        dialog.setWindowTitle("Fehler!")
        dialog.setText("Es wurden keine Bounding-Boxen gefunden!")
        dialog.exec()
        return
    interpreted_array = interprete(classification_result)
    if not interpreted_array:
        return
